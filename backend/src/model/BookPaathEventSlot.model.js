...
const findBookedSlotById = async (idBookSlot) => {
  try {
    let bookedSlot = await BookSlotPaath.findOne({
      where: { idBookSlot: idBookSlot },
    });
    return bookedSlot;
  } catch (err) {
    console.log(err);
...
const findAllBookedSlot = async (paath) => {
  try {
    let allbookedSlot = await BookSlotPaath.findAll({
      where: { PaathIdPaath: paath },
      include: ["User", "Paath"],
      raw: true,
      nest: true,
    });
...
const updatedBookedSlotbyId = async (bookedSlot) => {
  try {
    ...
    await BookSlotPaath.update(bookedSlot, {
      where: { idbookSlot: bookedSlot.idbookSlot },
    }).then((res) => {
...
const deleteBookedSlotById = async (idBookSlot) => {
  try {
    ...
    await BookSlotPaath.destroy({ where: { idBookSlot: idBookSlot } }).then(
      (res) => {
...
module.exports = {
  ...
  findAllBookedSlot,
  findBookedSlotById,
  updatedBookedSlotbyId,
  deleteBookedSlotById,
};
